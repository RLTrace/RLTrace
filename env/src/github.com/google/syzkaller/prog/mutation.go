...
func (p *Prog) Mutate(rs rand.Source, ncalls int, ct *ChoiceTable, corpus []*Prog) {
	...
	i := 0
	for stop, ok := false, false; !stop  && i < 10 ; stop = ok && r.oneOf(3) {
		switch {
		case r.oneOf(5):
...
func chooseCall(p *Prog, r *randGen) (*Call, bool, int) {
	...
	idx := randomChoice(callPriorities, r)
	return p.Calls[idx], true, idx
}
