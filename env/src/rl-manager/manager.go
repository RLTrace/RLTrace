...
func RunManager(cfg *mgrconfig.Config, target *prog.Target, sysTarget *targets.Target, syscalls []int) {
	...
	mgr := &Manager{
		...
		progqueue:        newProgQueue(),
	}

	log.Logf(0, "loading corpus...")
	os.Remove(filepath.Join(cfg.Workdir, "corpus.db"))
...
func (mgr *Manager) sendProgram(maxlen *int, reply *rpctype.ProgramArgs){
	lenth := *maxlen
	for item := mgr.progqueue.dequeue(wantIn); item != nil && reply.Length <= lenth; {
		program := item.(*ProgramIn)
		reply.Length++
		reply.Programs = append(reply.Programs, rpctype.Program{
			Trace:    program.Trace,
			Name:     program.Name,
			Coverage: []cover.Cover{},
		})
		item = mgr.progqueue.dequeue(wantIn)
	}
}

func (mgr *Manager) getCoverage(reply *rpctype.ProgramRes){
	for _, program := range reply.Programs{
		mgr.progqueue.enqueue(&ProgramOut{
			Name: program.Name,
			Trace: program.Trace,
...
func removeDuplicateElement(traces [][]int) [][]int{
	results := make([][]int, 0, len(traces))
	temp := map[string]struct{}{}
	for _, item := range traces {
		var tmp_slice []string
		for _, i := range item{
			tmp_slice = append(tmp_slice, strconv.Itoa(i))
		}
		key := strings.Join(tmp_slice, ",")
		if _, ok := temp[key]; !ok{
			temp[key] = struct{}{}
...
func (mgr *Manager) newInput(inp rpctype.RPCInput, sign signal.Signal) {
	...
	if old, ok := mgr.corpus[sig]; ok {
		...
		//log.Logf(0, "Length of Corpus is %d\n", len(mgr.corpus))
