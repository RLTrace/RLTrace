...
func (mgr *Manager) initHTTP() {
	...
	http.HandleFunc("/sendProgram", mgr.httpProgram)
	http.HandleFunc("/getCover", mgr.httpGetCover)
	http.HandleFunc("/getSyscall", mgr.httpGetSys)
...
func (mgr *Manager) httpProgram(w http.ResponseWriter, r *http.Request) {
	...
	var formData []Program
	json.Unmarshal(body, &formData)
	for _, value := range formData {
		log.Logf(0, "Get Program %v\n", value.Name)
		mgr.progqueue.enqueue(&ProgramIn{
			Name:  value.Name,
			Trace: value.Trace,
			Retry: 0,
		})
...
func (mgr *Manager) httpGetCover(w http.ResponseWriter, r *http.Request){
	type Item struct{
		Name string
		Trace []int
		Coverage []int
	...
	var output []Item
	for item := mgr.progqueue.dequeue(wantOut); item != nil && len(output) <= 1000; {
		mgr.progqueue.print()
		program := item.(*ProgramOut)
		var coverage []int
		for _, item := range program.Coverage{
			coverage = append(coverage, len(item.Serialize()))
		}
		output = append(output, Item{Name: program.Name, Trace: program.Trace,
			Coverage: coverage})
		log.Logf(0, "Coverage of %v is %v\n", program.Name, coverage)
		item = mgr.progqueue.dequeue(wantOut)
	}
...
func (mgr *Manager) httpGetSys(w http.ResponseWriter, r *http.Request){
	type Call struct{
		Name string
		ID int
	}
	...
	result, _ := json.Marshal(syscalls)
	w.Write(result)
}
